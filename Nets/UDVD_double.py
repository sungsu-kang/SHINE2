'''
code from : https://github.com/sreyas-mohan/udvd/blob/main/models/udvd.py
'''
...
class shift_double(nn.Module):
    def __init__(self):
        super().__init__()
        self.shift_down = nn.ZeroPad2d((0,0,2,0))
        self.crop = crop_double()
...
class Conv(nn.Module):
    def __init__(self, in_channels, out_channels, bias=False, blind=True, blind_first=False):
        super().__init__()
        self.blind = blind
        if blind:
            self.shift_down = nn.ZeroPad2d((0,0,1,0))
        ...
        if blind_first:
            self.shift_down = nn.ZeroPad2d((0,0,2,0))
            self.crop = crop_double()
            self.replicate = nn.ReplicationPad2d(2)
...
class ENC_Conv(nn.Module):
    def __init__(self, in_channels, mid_channels, out_channels, bias=False, reduce=True, blind=True, blind_first=False):
        super().__init__()
        self.reduce = reduce
        self.conv1 = Conv(in_channels, mid_channels, bias=bias, blind=blind, blind_first=blind_first)
...
class Blind_UNet(nn.Module):
    def __init__(self, n_channels=3, n_output=96, bias=False, blind=True, blind_first=False):
        super().__init__()
        self.n_channels = n_channels
        self.bias = bias
        self.enc1 = ENC_Conv(n_channels, 48, 48, bias=bias, blind=blind, blind_first=blind_first)
...
class BlindVideoNet_e(nn.Module):
    def __init__(self, channels_per_frame=3, out_channels=9, bias=False, blind=True, sigma_known=False):
        super().__init__()
        self.c = channels_per_frame
        self.out_channels = out_channels
        self.blind = blind
        self.sigma_known = sigma_known
        self.rotate = rotate()
        self.denoiser_1 = Blind_UNet(n_channels=3*channels_per_frame, n_output=32, bias=bias, blind=blind, blind_first=True)
